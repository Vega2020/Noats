...
function getRecipe(tag) {
  ...
  $.ajax({
    ...
    const recipeLink = $("<a>").attr(
      "href",
      response.results[random].sourceUrl
    );
    ...
    const currentRecipe = response.results[random].id;

    const recipePicture = `https://spoonacular.com/recipeImages/${currentRecipe}-556x370.jpg`;
    ...
    function getIngredients() {
      ...
      const ingredientQuery = `https://api.spoonacular.com/recipes/${currentRecipe}/ingredientWidget.json?apiKey=c30cd056ba1c4e459950da3b71b83d82`;
